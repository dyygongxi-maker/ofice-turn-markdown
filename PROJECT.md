...
正式安装包采用 Inno Setup 6，生成单文件 `廾匸转换-Setup-0.3.0.exe`。它按当前 Windows 用户安装到 `%LocalAppData%\Programs\廾匸转换`，提供开始菜单、可选桌面快捷方式和标准卸载入口；不需要管理员权限，安装包不进入 Git。程序和安装程序共用项目内生成的蓝色文档转换图标，确保快捷方式、安装向导和 Windows 资源管理器中的识别一致；最新含图标的安装包已于 2026-09-05 重新构建并验证。
