...
- 解析与输出解耦：所有格式适配器先生成统一中间文档模型，再由输出渲染器生成 Markdown、JSON 或 HTML。
...
Tkinter desktop UI
  ...
  -> format adapter (DOCX | PPTX | XLSX | PDF | TXT | CSV | Markdown | JSON)
  -> normalized document model
  -> Markdown/JSON/HTML renderer + asset exporter + optional WPS-first visual exporter
...
- 输出：用户选择的本地目录；包含用户选择的 Markdown、JSON 或 HTML、资源、报告，以及可选的 PPT 页面预览和 PDF。
- 网络：MVP 运行时不发起外部网络请求。
...
```text
<source-name>-markdown/、<source-name>-json/ 或 <source-name>-html/
  index.md
  markdown/
    <source-name>.md
...
HTML 使用同样的事务目录结构，根目录后缀为 `-html`，正文位于 `html/<源文件名>.html`。HTML 仅接受 DOCX、PPTX、XLSX 输入；CSV、Markdown 和 JSON 继续通过统一模型转换为 Markdown 或 JSON。
...
- `ConversionOptions` 承载可选 Obsidian 输出和受控输出格式；默认 Markdown 协议保持不变，JSON 与 HTML 使用独立输出目录。JSON 输入必须通过 `schema_version: 1`、块字段与表格字段校验后才可进入模型。
