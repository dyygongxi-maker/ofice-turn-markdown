...
推荐以 **Python 转换核心 + Tkinter Windows 原生桌面 UI** 构建 MVP。该选择直接满足纯本地运行、可靠的文件/目录选择和未来 Windows 分发需求，同时使解析与 UI 维持明确边界。
...
| 方案 | 优点 | 缺点 | 结论 |
| --- | --- | --- | --- |
| Python + Tkinter | 标准库、原生文件选择；单语言；适合离线与打包 | UI 定制能力有限 | 推荐 |
